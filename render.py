...
def draw_train(screen, train, camera):
    ...
    # Pozycja pociągu na odcinku segmentu
    ...
    # Rysuj lokomotywę
    pygame.draw.circle(screen, (200, 0, 0), screen_pos, 10)

    # Rysuj wagon za lokomotywą
    offset = 15
...
def draw_station(screen, station, camera):
    ...
    # Draw passengers above the station
    max_dots = 8
    dot_radius = 4
    spacing = 10
    offset_x = 10
    ...
    for i in range(min(station.passengers, max_dots)):
        col = i % 2
        row = i // 2
    ...
    # Overload visualization if more than max passengers
    if station.passengers > max_dots:
        percent = min(station.overload_timer / 5.0, 1.0)  # 5s do przepełnienia
        s = pygame.Surface((50, 50), pygame.SRCALPHA)
        pygame.draw.circle(s, (0, 0, 0, 100), (25, 25), 25)  # dark translucent circle
    ...
    # Red highlight if station is overloaded
